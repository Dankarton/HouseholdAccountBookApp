...
public class Expenses extends BOP implements HasCategory, HasWallet {
    ...
    public void onAfterInsert(long newId) {
        ...
        RepositoryManager.getInstance().updateMonthlyBalanceDelta(this.getDate(), this.walletId, -1 * Math.abs(this.getAmount()));
    }

    @Override
    public void onAfterUpdate(DatabaseEntity before) {
        RepositoryManager.getInstance().updateMonthlyBalanceDelta(((Expenses) before).getDate(), this.walletId, Math.abs(((Expenses) before).getAmount()));
        RepositoryManager.getInstance().updateMonthlyBalanceDelta(this.getDate(), this.walletId, -1 * Math.abs(this.getAmount()));
    }
    @Override
    public void onAfterDelete() {
        RepositoryManager.getInstance().updateMonthlyBalanceDelta(this.getDate(), this.walletId, Math.abs(this.getAmount()));
    }
}
