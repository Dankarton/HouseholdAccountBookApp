...
public class Purchase extends BOP implements HasCategory {
    ...
    public void onAfterInsert(long newId) {
        ...
        ArrayList<Expenses> newExpList = RepositoryManager.getInstance().getDataById(PaymentMethod.class, this.paymentMethodId).makeExpenses(this);
        for (Expenses exp: newExpList) {
            RepositoryManager.getInstance().setDataSafely(exp);
        }
    }
    @Override
    public void onAfterUpdate(DatabaseEntity before) {
        // 自身に連なっていたExpensesを全て削除
        ArrayList<Expenses> oldExpList = RepositoryManager.getInstance().getChildExpensesList(this);
        for (Expenses exp : oldExpList) {
            RepositoryManager.getInstance().deleteDataSafely(exp);
        }
